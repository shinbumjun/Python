...
앙상블 학습(Ensemble)은 여러 개의 분류기를 생성하고,
...
x_train, x_test, y_train, y_test = train_test_split(cancer.data, cancer.target, 
                                                    test_size=0.2, random_state=12) # 8대2
...
voting_model = VotingClassifier(estimators=[('LR', logi_regression),('KNN',knn),('Decision',dec_model)],
                                voting='soft') # 투표방식은 평균을 이용하여(soft)

classifiers = [logi_regression, knn, dec_model] # 성능을 확인하기 위해서 

for classifier in classifiers: # 개별 모델의 학습 및 평가
    classifier.fit(x_train, y_train)
    pred = classifier.predict(x_test)
    class_name = classifier.__class__.__name__ # class의 이름을 얻기
    print('{0} 정확도:{1:.4f}'.format(class_name, accuracy_score(y_test, pred))) # 1번째 소수4자리까지
