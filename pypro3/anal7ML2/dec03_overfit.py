...
# 정확도 100% 겁나 찝찝~~ : 과적합 문제 발생 (모델의 포용성에 위배)
# 모의고사 문제와 본고사 문제가 똑같음
# 모델의 포용성을 위해 과적합 문제를 해결하는 방안

print('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
print('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
print('과적합 방지 목적 처리 1') # 전통적인 방법이 딥러닝에서도 필요함
...
# 모델 학습시 데이터의 편중을 방지하고자 학습 데이터를 쪼개 학습과 평가를 병행 
# 교차검증 중 가장 보편적인 방법으로 k-fold
from sklearn.model_selection import KFold
import numpy as np
features = iris.data
label = iris.target
dt_clf = DecisionTreeClassifier(criterion = 'entropy', random_state = 123) # 불순물
...
print('대출 사기 데이터인 경우 정상과 사기 데이터가 비율적으로 편향된 경우가 많다. 이메일, 강우 여부')    
...
for train_index, test_index in skfold.split(features, label): # kfold -> skfold, ()안에 label만 수정 및 추가 하면됨
    xtrain, xtest = features[train_index], features[test_index]
    ytrain, ytest = label[train_index], label[test_index]
...
print('cross_val_score를 이용하면 교차검증을 쉽게 할 수 있다.')
from sklearn.model_selection import cross_val_score

data = iris.data
label = iris.target

score = cross_val_score(dt_clf, data, label, scoring='accuracy', cv = 5) # 5번 돌리기
...
print('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
print('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
print('과적합 방지 목적 처리 3 : 어려운 내용')
print('GridSearchCV : 교차검증과 최적의 속정(하이퍼 파라미터)을 위한 튜닝을 한 번에 처리')
from sklearn.model_selection import GridSearchCV

print('여러개의 속성 값 중 max_depth, min_samples_split에 대해서 최적의 값 찾기')
...
print('GridSearchCV 최적 파라미터 : ', grid_tree.best_params_) # {'max_depth': 3, 'min_samples_split': 2}
print('GridSearchCV 최고 정확도 : ', grid_tree.best_score_) # 0.942
# dt_clf = DecisionTreeClassifier(..., max_dapth = 3, min_samples_split = 2, ...)
...
print(pred)
print('모델 성능(정확도) : ', accuracy_score(y_test, pred)) # 0.955
