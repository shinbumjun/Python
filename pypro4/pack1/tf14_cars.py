...
print('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
print('train / test split : sample() 함수')
...
print('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
print('fit 전에 predict() 가능 : 결과는 기대하지 말자') 

epochs = 5000 # 쫄지 말자! 조기종료를 할 것이므로

early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', mode = 'auto', patience=5) # validation_split로 인해서 val_loss 가능

history = model.fit(st_train_data, train_label, batch_size = 32, epochs=epochs,
                    validation_split=0.2, verbose=1, callbacks=[early_stop]) # 조건에 맞으면 5000번 안돔
df = pd.DataFrame(history.history)
print(df.columns)
print(df.head(2))

print('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
print('모델 성능 (mae, mse) 시각화')
